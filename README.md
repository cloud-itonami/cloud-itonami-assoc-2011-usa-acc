# cloud-itonami-assoc-2011-usa-acc

Industry rule/policy-statement catalog for the **American Chemistry
Council** (ACC) — the FIRST entry aligned to **ISIC 2011** (manufacture
of basic chemicals), a new industry code for this family. Alongside
...
actuation actor. It proposes or executes nothing on ACC's behalf.

Coverage is reported through bounded `entry-count`, `association-covered?`,
and `by-topic-*` operations. Unknown associations have no spec-basis.
...
[americanchemistry.com](https://www.americanchemistry.com/)'s own
pages: **Our 150 Years History** (confirms ACC's founding year, 1872,
originally as the Manufacturing Chemist Association of the United
States) and **Responsible Care Overview** (the chemical industry's
flagship safety/sustainability self-regulatory program, launched in
the U.S. in 1988). Neither page states a specific month/day, so both
`:association-rule/established-date` values are deliberately
year-only rather than invented full dates.

## License

AGPL-3.0-or-later (matches the `cloud-itonami-iso3166-*` /
`-municipality-*` / `-assoc-*` / `-lei-*` convention). Policy text
itself remains ACC's; this repo stores only citation metadata
